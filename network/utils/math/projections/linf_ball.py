...
def get_linf_projection_threshold(weight, cuda):
    with torch.no_grad():
        if not cuda:
            ...
            candidate_ks = (
                candidate_ks.float()
                + (1.0 / (2 * partial_sums.shape[1]))
                * (indices + torch.tensor(1).float()).float()
            )
            _, ks = torch.max(candidate_ks.float(), dim=1)
            ks = ks.float()
            index_ks = torch.cat(
                (
                    torch.arange(end=weight.shape[0]).unsqueeze(-1).float(),
                    ks.unsqueeze(1),
                ),
                dim=1,
            ).long()
            ...
            thresholds = (
                partial_sums[index_ks[:, 0], index_ks[:, 1]] - torch.tensor(1).float()
            ...
            candidate_ks = (
                candidate_ks.float().cuda()
                + (1.0 / (2 * partial_sums.shape[1]))
                * (indices + torch.tensor(1).float().cuda()).float()
            )
            _, ks = torch.max(candidate_ks.float(), dim=1)
            ks = ks.float().cuda()
            index_ks = torch.cat(
                (
                    torch.arange(end=weight.shape[0]).unsqueeze(-1).float().cuda(),
                    ks.unsqueeze(1),
                ),
                dim=1,
            ).long()
            ...
            thresholds = (
                partial_sums[index_ks[:, 0], index_ks[:, 1]]
                - torch.tensor(1).float().cuda()
...
def project_on_linf_ball(weight, cuda):
    with torch.no_grad():
        ...
        signs[signs == 0] = 1
        projected_weights = signs * torch.clamp(
            torch.abs(weight) - thresholds.unsqueeze(-1), min=torch.tensor(0).float()
        )
...
def scale_on_linf_ball(weight, scale_all=True, cuda=False):
    with torch.no_grad():
        row_scaling_factors = get_l_inf_row_normalization_factors(
            weight, scale_all, cuda
        )
